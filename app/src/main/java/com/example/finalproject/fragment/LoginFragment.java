...
public class LoginFragment extends Fragment {
    ...
    private void loginUser() {
        ...
        if (TextUtils.isEmpty(Email)){
            ...
            New.appPreference.showToast("Password  may be at least 6 characters long.");
        } else {
            Call<User> userCall = New.serviceApi.doLogin(Email, Password);
            userCall.enqueue(new Callback<User>() {
                @Override
                public void onResponse(Call<User> call, Response<User> response) {
                    if (response.body().getResponse().equals("data")){
                        New.appPreference.setLoginStatus(true); // set login status in sharedPreference
                        loginFromActivityListener.login(
                                response.body().getName(),
                                response.body().getEmail(),
                                response.body().getCreatedAt());
                    } else if (response.body().getResponse().equals("login_failed")){
                        New.appPreference.showToast("Error. Login Failed");
                        emailInput.setText("");
